...
def create_chunks(week_to_data, missing_weeks):
    ...
    """ This function takes in the standard format
    weekly data and information about the missing weeks
    and returns a list of continuous time periods,
     breaking where there are missing parts in the data."""
...
def generate_dataset(chunks, train_size, predict_size):
    ...
    # which are pairs of (x,y) training data points,
    # where dim(x) = (20, trainsize), dim(y) = (20, testsize)

    # TODO change this to make it like ptb, a sliding window rather than predicting a seq
    X = []
    Y = []
    for chunk in chunks:
        ...
        for i in range(chunk_length - (train_size + predict_size) -1):
            ...
            X.append(np.array(thisX).astype(np.float32))
            Y.append(np.array(thisY).astype(np.float32))

    data = zip(X,Y)
    random.shuffle(data)
    datapoints = len(data)

    train = data[0:int((round(datapoints*0.7)))]
    val = data[int((round(datapoints*0.7))):int((round(datapoints*0.15)))]
    test = data[int((round(datapoints*0.85)))::]
...
if __name__ == "__main__":
    ...
    os.chdir("../")
    preprocess(24,24,"data/energy/preprocess/NN_data_debug.pkl")